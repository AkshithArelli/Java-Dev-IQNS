...
1️⃣ What is HashMap internally?

At the core, HashMap is:
...
Each bucket stores:
	...
	•	a Red-Black Tree (Java 8+ optimization)



2️⃣ What does a Node contain?
...
3️⃣ What happens when you do put(key, value)?
...
```
Java further spreads the hash to reduce collisions:
```
hash = hash ^ (hash >>> 16);
```
Why?
👉 To mix higher bits with lower bits for better bucket distribution.
...
➡️ Create new Node and store it
```
Bucket[index] → [apple,100]
```



Case 2: Bucket is NOT empty (collision)
Now HashMap traverses the bucket:

For each existing node:

	1.	Compare hash
	2.	Compare key using equals()
...
➡️ No new node created



🔸 If key does NOT exist

➡️ Add new node at end of list
```
Bucket[index] → [apple,100] → [banana,50]
```



How HashMap stores data inside a bucket

Each bucket stores a linked list (or tree, explained later) of Node objects.
...
Bucket[index]
    ↓
[ hashA | key=A | value=10 | next ] → [ hashB | key=B | value=20 | next ] → null
```
4️⃣ Treeification (Java 8+ magic)
...
➡️ Linked List converts to Red-Black Tree
...
5️⃣ What happens during get(key)?
...
⚠️ hashCode() + equals() both must work correctly



6️⃣ What is resizing (rehashing)?
...
➡️ Capacity doubles → 32

➡️ All entries are rehashed

⚠️ Rehashing is expensive
