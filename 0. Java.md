...
A `HashMap` in Java operates on the principle of **Hashing**. It uses an internal array-based data structure combined with Linked Lists or Red-Black Trees to handle collisions.

---

## 1. The Core Structure: The Entry Array
Internally, a `HashMap` maintains an array called `table`. Each element in this array is referred to as a **"Bucket."**

* Each bucket stores a `Node<K, V>` (or `Entry`).
* A `Node` contains four fields:
    1. `int hash`
    2. `K key`
    3. `V value`
    4. `Node<K, V> next` (Points to the next node in case of a collision)



---

## 2. How `put(K key, V value)` Works
When you call the `.put()` method, the following steps occur:

### A. Hash Code Calculation
Java calls the `hashCode()` method on your key.

### B. Hashing Function
To prevent poor `hashCode` implementations from causing too many collisions, Java applies a secondary hash function to spread the bits:
$$hash = (h = key.hashCode()) \oplus (h \gg 16)$$

### C. Index Calculation
The hash is converted into an array index using a bitwise AND operation (which is faster than the modulo operator):
$$index = (n - 1) \& hash$$
*(where $n$ is the current capacity of the array)*.

### D. Handling the Insertion
* **Empty Bucket:** If the index is empty, the Node is placed there directly.
* **Collision:** If a Node already exists at that index, Java checks the keys using `.equals()`:
    * If `key.equals(newKey)` is **true**, the existing value is overwritten.
    * If **false**, the new Node is added to the **Linked List** (or Tree) at that bucket.

---

## 3. Collision Handling: From Linked Lists to Trees
In older versions of Java, collisions were handled strictly with Linked Lists. However, if many keys hash to the same index, performance drops from $O(1)$ to $O(n)$.

**In Java 8 and later:**
* **Thresholding:** Once a bucket reaches a size of **8** nodes, and the total map capacity is at least **64**, the Linked List is converted into a **Red-Black Tree**.
* **Performance Improvement:** This ensures that even in the worst-case scenario of high collisions, lookup time is reduced from $O(n)$ to $O(\log n)$.
...
3.  Go to that bucket.
4.  Iterate through the Nodes (using `.equals()`) until a match is found or the list/tree ends.

---

## 5. Capacity, Load Factor, and Rehashing
To maintain performance, a `HashMap` must resize itself before it becomes too crowded.

* **Initial Capacity:** The default starting size of the array is **16**.
* **Load Factor:** The default is **0.75**. This means the map will resize when it is 75% full.
* **Rehashing:** When the threshold is hit, the array size doubles. Because the index calculation formula depends on the array size ($n-1$), every existing element must be re-calculated and moved to its new appropriate index.

---

## Summary Table: Time Complexity

| Operation | Best Case | Worst Case (Post Java 8) |
| :--- | :--- | :--- |
| **Search** | $O(1)$ | $O(\log n)$ |
| **Insertion** | $O(1)$ | $O(\log n)$ |
| **Deletion** | $O(1)$ | $O(\log n)$ |

> **Note:** For the best performance, always ensure your keys provide a well-distributed `hashCode()` and a consistent `equals()` implementation.
