...
```
A method reference is a short way of writing lambda expression
when lambda is only calling an existing method.

Instead of writing method call inside lambda
You can directly reference the method.
```
...
Perfect — let’s go step-by-step, internally, the way interviewers expect you to explain it.
I’ll keep it simple, visual in words, and chronological 👇
