...
Here’s your content properly **formatted in clean Markdown**, without losing any details 👇

---

# 📌 HashMap Internal Working (Formatted Notes)

## 1. 🧱 The Core Structure: Entry Array

Internally, a `HashMap` maintains an array called **`table`**.

* Each element in this array is called a **Bucket**
* Each bucket stores a `Node<K, V>` (also known as Entry)
...
## 2. ⚙️ How `put(K key, V value)` Works

When `.put()` is called, the following steps occur:
...
### 🔹 Step 2: Hashing Function (to reduce collisions)
...
* **Collision Case**
  ...
  * If `false` → add to Linked List in that bucket


## 3. 🔄 Collision Handling: Linked List → Tree
...
➡️ Convert Linked List → **Red-Black Tree**
...
Retrieval is reverse of insertion:
...
## 5. 📦 Capacity, Load Factor & Rehashing
...
➡️ Resize happens when:
[
size \ge capacity \times load\ factor
]


### 🔹 Rehashing

* Array size doubles (e.g., 16 → 32)
* All elements are recalculated and moved


## 📊 Summary Table

| Operation | Best Case | Worst Case (Java 8+) |
| --------- | --------- | -------------------- |
| Search    | (O(1))    | (O(\log n))          |
