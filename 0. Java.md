...
Used in method signature to indicate the method may throw exceptions. It tells the caller of the method to handle the exception.
```java
...
Definition: Deadlock is a situation where two or more threads are permanently blocked, each waiting for a resource held by the other.
...
Definition: A reentrant lock is an advanced locking mechanism from java.util.concurrent.locks that allows a thread to acquire the same lock multiple times without blocking itself.
...
10️⃣ One-line summary (memorize this)

HashMap stores key-value pairs in an array of buckets, uses hashCode() to find the bucket, equals() to find the exact key, handles collisions using linked lists or trees, and resizes itself when load factor is exceeded.
