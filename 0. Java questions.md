...
✔ Even after return statement inside try/catch
...
Purpose		Convert object → bytes		Convert bytes → object
...
wait()		Go to sleep		✅ Yes	—
notify()	Wake up one		❌ No (until synchronized block ends)	One thread
notifyAll()	Wake up all		❌ No (until synchronized block ends)	All threads
```
...
### 🚀 Deadlock Prevention Techniques (Interview Gold)
