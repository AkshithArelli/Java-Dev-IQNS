...
🔧 Comparable — details & example

When to use

	•	Use when your class has a single, natural order (e.g., String alphabetical, Integer numeric, Date chronological).
...
compareTo must be:
	...
	•	0 if objects are equal in ordering,
	•	negative if this < other,
	•	positive if this > other.

### It should be consistent with equals: if a.equals(b) then a.compareTo(b) should be 0 (recommended).

Pitfall: Don’t do return this.id - other.id; — it can overflow for large ints. Use Integer.compare.

⸻

🧠 Comparator — details & examples

When to use

	•	When you need different orderings for the same class (salary, name, age).
	•	When you cannot modify the class (3rd-party class).
	•	When you need a one-off ordering without changing the class.

Typical forms

	•	Separate class implementing Comparator<T>
	•	Anonymous inner class
	•	Lambda expression (Java 8+)
	•	Use static factory helpers: Comparator.comparing(...), thenComparing(...), reversed(), nullsFirst() etc.
...
Implemented by			The class whose objects are being ordered			Separate class, lambda, or static factory
Purpose					Natural/default ordering (e.g., String, Integer)	Alternative/custom orderings (salary, name, etc.)
Multiple orderings		No (only one compareTo)								Yes (many different comparators)
Location				Inside the class									Outside the class (or as lambda/static method)
Java 8 helpers				—												Comparator.comparing, thenComparing, reversed, nullsFirst
Use with				Collections.sort(list) or Arrays.sort(array)		Pass to Collections.sort(list, comparator) or 
						uses compareTo if no comparator provided			list.sort(comparator)
Consistency 			with equals	Should be consistent with equals ideally	Can be independent from equals
```

⸻

🔁 Java 8 Comparator helpers (very useful)

	•	Comparator.comparing(Function<T,U>)
	•	Comparator.comparingInt(ToIntFunction<T>), comparingDouble, etc.
	•	thenComparing(...) for tie-breakers
	•	reversed() to flip order
	•	nullsFirst() / nullsLast() to handle null elements
...
🧾 Complexity & behavior
	...
	•	Sorting with Comparable or Comparator uses the same algorithm (TimSort for Collections.sort/List.sort in modern JDK).
	•	Time complexity: O(n log n) comparisons (where each comparison cost depends on compare method).
	•	Space: TimSort uses O(n) worst-case temporary space but typically less; same for either approach.

⸻

### 🔐 Consistency with equals

	•	If compareTo or compare yields 0 for two objects they are considered equal in sort order. If that conflicts with equals, you can get surprising behavior with sorted sets/maps (TreeSet, TreeMap) — use caution.
	•	Example: If compareTo compares only id but equals uses id + name, TreeSet may treat two objects with same id but different names as duplicates.

⸻

🧨 Common pitfalls & interview traps

	•	Integer subtraction overflow: don’t use a - b for compare; use Integer.compare(a,b).
	•	Inconsistent with equals: can break sets/maps ordering semantics.
	•	Mutable keys: objects used in a sorted set/map should not change fields used for comparison while inside the collection.
	•	Null handling: decide nullsFirst or nullsLast if collection can contain nulls.
...
✅ When to choose which
	...
	•	Use Comparable when:
	•	There is a clear natural ordering.
	•	You control the class and it makes sense to embed the order.
	•	Use Comparator when:
	•	You need multiple orderings.
	•	You can’t modify the class (library classes).
	•	You want dynamic order (e.g., comparator chosen at runtime).
	•	You want to chain complex ordering rules using Java 8 helpers.

⸻

Full working example (pasteable)
```java
import java.util.*;
...
class Employee implements Comparable<Employee> {
    ...
    public String toString() {
        return String.format("Employee{id=%d,name=%s,sal=%d}", id, name, salary);
    }
}
...
public class CompareExample {
    public static void main(String[] args) {
        List<Employee> list = Arrays.asList(
            ...
            new Employee(1, "Alice", 85000),
            new Employee(2, "Bob", 65000)
        );

        // 1) Comparable -> sort by id (natural)
        Collections.sort(list);
        System.out.println("By id (Comparable): " + list);

        // 2) Comparator -> sort by salary asc
        list.sort(Comparator.comparingInt(Employee::getSalary));
        System.out.println("By salary (Comparator): " + list);

        // 3) Composite comparator -> salary desc, then name asc
        list.sort(Comparator.comparingInt(Employee::getSalary).reversed()
                            .thenComparing(Employee::getName));
        System.out.println("By salary desc then name: " + list);

        // 4) Comparator via lambda
        list.sort((e1, e2) -> e1.getName().compareTo(e2.getName()));
        System.out.println("By name (lambda): " + list);
    }
}
```
Output (example):
```
By id (Comparable): [Employee{id=1,name=Alice,sal=85000}, Employee{id=2,name=Bob,sal=65000}, Employee{id=3,name=Charlie,sal=75000}]
By salary (Comparator): [Employee{id=2,name=Bob,sal=65000}, Employee{id=3,name=Charlie,sal=75000}, Employee{id=1,name=Alice,sal=85000}]
By salary desc then name: [Employee{id=1,name=Alice,sal=85000}, Employee{id=3,name=Charlie,sal=75000}, Employee{id=2,name=Bob,sal=65000}]
By name (lambda): [Employee{id=1,name=Alice,sal=85000}, Employee{id=2,name=Bob,sal=65000}, Employee{id=3,name=Charlie,sal=75000}]
```

⸻

### 🧠 summary (short bullets you can memorize)

	•	Comparable for natural ordering, implement compareTo inside class.
	•	Comparator for custom/multiple orderings, implement externally or use lambdas.
	•	Use Integer.compare, Double.compare to avoid overflow.
	•	Java 8 Comparator helpers (comparing, thenComparing, reversed) make complex comparisons concise.
...
### When you use a sorted collection in Java — like a TreeSet or TreeMap — it doesn’t use equals() to check for duplicates. Instead, it uses comparison (compareTo or Comparator) to decide:

“Are two elements equal or not?”

⸻

⚠️ Why this can cause problems
...
class Employee implements Comparable<Employee> {
    ...
    // equality checks both id and name
    @Override
    public boolean equals(Object o) {
...
set.add(new Employee(101, "Akshith"));
set.add(new Employee(101, "Yashwanth"));
...
Two different employees (since names differ).

😬 What actually happens:
```
[101-Akshith]
```
The second one (101, "Yashwanth") is ignored.

Because compareTo() returned 0 (same id),

TreeSet thought “they are equal” — even though equals() says they aren’t.

⸻

🧠 Meaning of “consistent with equals”
...
then compareTo() (or compare()) must return 0.

That keeps your sorting and equality logic aligned — no surprises with TreeSet/TreeMap.

⸻

✅ Correct version

Make both use the same rule:
```java
@Override
public int compareTo(Employee other) {
    int idCompare = Integer.compare(this.id, other.id);
    if (idCompare != 0) return idCompare;
    return this.name.compareTo(other.name); // now matches equals()
}
```
Now, both:
...
say: “Two employees are equal only if both id and name are equal.”
...
```
Concept	Meaning
equals()							Decides if two objects are equal (used by HashSet, HashMap)
compareTo() / Comparator.compare()	Decides ordering and equality for sorted collections (TreeSet, TreeMap)
...
⭐ Fail-Fast vs Fail-Safe — Short & Perfect Explanation
...
Collections					ArrayList, HashMap		ConcurrentHashMap, COWArrayList
Behavior on modification	Throws CME				No exception
Works on					Original collection		Cloned copy
Safe for multithreading		❌ No					✔ Yes
Performance					Fast					Slower (copying cost)
```
...
Fail-Fast iterators throw ConcurrentModificationException if the collection is modified during iteration because they work on the original structure.
Fail-Safe iterators do not throw exceptions because they work on a copy of the collection (like in ConcurrentHashMap or CopyOnWriteArrayList).
